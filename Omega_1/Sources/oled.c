...
//数据处理
//原则全部看作0000.00处理
void dis_num(u8 n,u8 y,int a)//Achtung!---bei Herrn Yang
{
	u32 raw;
	int i;
	u8 digit=0;
...
if(n)
	{OLED_ShowNum(40+11*(4-digit),y,raw,digit,22);
	if(x)
		OLED_ShowChar(84,y,'-');
	else
		OLED_ShowChar(84,y,'+');
	}
else {OLED_ShowNum(11*(4-digit),y,raw,digit,22);
 if(x)
	OLED_ShowChar(44,y,'-');
 else
	OLED_ShowChar(44,y,'+');
...
/*void dis_num(u8 y,u32 a)
{
	...
	OLED_ShowNum(30+11*(4-digit),y,raw,digit,22);
}*/
void dis_decimals(float a,int digitt,u8 y)//digit:保留的小数位数//Aber herrlich---bei Herrn Yang
{
	int raw=a;
	int raww=raw;
	u8 decimals;
	...
	decimals=(int)a%t;//decimals=(u8)a%t;数制转换会出问题！denn:%只用于int！
	
	OLED_ShowNum(30+11*(4-digit),y,raww,digit,22);
	OLED_ShowChar(74,y,'.');
	OLED_ShowNum(82,y,decimals,digitt,22);
}
